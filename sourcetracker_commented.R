...
"sourcetracker" <- function(train, envs, rarefaction_depth=1000){
  ...
  if(sum(as.integer(train) != as.numeric(train)) > 0){
    stop('Data must be integers. Consider using "ceiling(datatable)" or ceiling(1000*datatable) to convert floating-point data to integers.')
  }
  envs <- factor(envs)
...
#   A more conservative value would be 0.001 or 0.01. (use proportion?)
...
"run.gibbs" <- function(sources, test, V, T, N,
  ...
  for(i in 1:N){
    ...
    for(j in 1:nrestarts){
      ...
      envcounts <- rep(beta, V)  ## prior counts to environment
      for(ix in 1:D){
      ...
      for(rep in 1:npasses){
        ...
        cnt <- 0
        for(ix in rand.ix){   # iterate over all sequences
          taxon <- tax.ix[ix]
